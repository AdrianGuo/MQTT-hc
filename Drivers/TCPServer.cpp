...
TCPServer::TCPServer(
    ...
    makeFunctor((threadFunctor_p) NULL, *this, &TCPServer::ListenThread);
    m_pListenThread->RegThreadFunctor(&m_ListenThreadFunctor);

    m_MessageThreadFunctor =
    makeFunctor((threadFunctor_p) NULL, *this, &TCPServer::MessageThread);
    ...
    // Note: Do like this then functor run on main thread.
    // So that, must carefully Lock data.
    m_pServerSockTimer = RTimer::getTimerInstance();
    m_keepaliveTimerFunctor =
    makeFunctor((TimerFunctor_p) NULL, *this, &TCPServer::HandleKeepAliveProcess);
...
TCPServer::Serve() {
    ...
    if(listen(m_idwSockfd, MAX_CLIENTS) < 0) { // Create queue for listening
        m_boIsServed = FALSE;
        return FALSE;
    }
 ...
 */
void_t
TCPServer::HandleKeepAliveProcess(
    void_p pBuffer
) {
