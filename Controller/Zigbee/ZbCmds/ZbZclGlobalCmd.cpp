...
ZbZclGlobalCmd::ProcRecvMessage(
    ...
    u8_t byCmdID = *pbyBuffer++;
    ++pbyBuffer; //payload length

    switch (byCmdID) {
        case ZCL_CMD_READ_RSP:
            ReadAttributeResponse(device, pbyBuffer);
            break;

        case ZCL_CMD_WRITE_RSP:
            WriteAttributeResponse(device, pbyBuffer);
            break;
...
ZbZclGlobalCmd::ReadAttributeRequest(
    ...
    if(device->Action.find(devInfo) == device->Action.end()) { return; }

    if((device->RealType) > 0 || (device.Modify()->Action[devInfo].DP_ClusterID == ZCL_CLUSTER_ID_GEN_BASIC)) {
        ...
        pZbPacket->Push(device.Modify()->Action[devInfo].DP_ClusterID >> 8);
        pZbPacket->Push(device.Modify()->Action[devInfo].DP_ClusterID & 0xFF);
        pZbPacket->Push(ZCL_CMD_READ);
        pZbPacket->Push(0x02); //Payload's length
        pZbPacket->Push(device.Modify()->Action[devInfo].DP_AttributeID & 0xFF);
        pZbPacket->Push(device.Modify()->Action[devInfo].DP_AttributeID >> 8);
