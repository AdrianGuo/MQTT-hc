...
//                    DEBUG2("Send req at count %d th for type %d.", m_mapEPInfor[wNwk].byEPCount, m_mapEPInfor[wNwk].mapType[i]);
                    ZbZclGlobalCmd::s_pInstance->ReadAttributeRequest(device, DeviceInfo::DI_Model);
                    ZbZclGlobalCmd::s_pInstance->ReadAttributeRequest(device, DeviceInfo::DI_Manufacturer);
                }
            }
        }
