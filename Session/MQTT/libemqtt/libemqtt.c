...
int mqtt_publish(mqtt_broker_handle_t* broker, const char* topic, const char* msg, uint16_t len, uint8_t retain) {
	return mqtt_publish_with_qos(broker, topic, msg, len, retain, 0, NULL);
}

int mqtt_publish_with_qos(mqtt_broker_handle_t* broker, const char* topic, const char* msg, uint16_t len, uint8_t retain, uint8_t qos, uint16_t* message_id) {
	uint16_t topiclen = strlen(topic);
	uint16_t msglen = len;//strlen(msg);
